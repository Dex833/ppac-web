...
function CashFlowView({ reportDoc }) {
  ...
  return (
    <>
      {/* Desktop: Use your existing beautiful table layout (already saved fields) */}
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full border border-gray-300 rounded text-sm mb-6">
            ...
            <tbody>
              ...
              <tr>
                <td className="p-2 border-b border-r border-gray-200" colSpan={3}>
              ...
              <tr>
                <td className="p-2 border-b border-r border-gray-200 italic">
                  Net Changes on Working Capital
                </td>
                <td className="p-2 border-b border-r border-gray-200">—</td>
                <td className="p-2 border-b text-right">{fmt(d.workingCapital)}</td>
              </tr>
              <tr className="font-semibold">
                <td className="p-2 border-t border-r border-gray-200">
              ...
              <tr className="font-bold bg-gray-50">
                <td className="p-2 border-t border-r border-gray-200">
                  Ending Balance Of Cash As Of {longDate(to)}
                </td>
                <td className="p-2 border-t border-r border-gray-200">—</td>
      ...
      <div className="sm:hidden space-y-2">
        <div className="card p-3">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
            <div>
              Net Change:{" "}
              <span className="font-mono">{fmt(s.netChangeCash)}</span>
            </div>
            <div>
              Ending Cash: <span className="font-mono">{fmt(s.endCash)}</span>
            </div>
          </div>
        </div>
      </div>
...
/* --------------------- Trial Balance (NEW mobile) --------------------- */

// Extract only the inner table from the saved snapshot HTML (remove headings).
function extractTableFromSnapshot(html) {
  if (!html) return "";
  // Try the "content div" we appended right before saving
  const mDiv = html.match(
    /<div[^>]*margin-top:10px[^>]*>([\s\S]*?)<\/div>/i
  );
  if (mDiv) return mDiv[1];
  // Fallback: get the first/last table
  const mTable = html.match(/<table[\s\S]*<\/table>/i);
  if (mTable) return mTable[0];
  return html;
}

// Trial Balance (saved in financialReports with type: "trial_balance")
function TrialBalanceView({ reportDoc }) {
  // TB saved period (our TB writer used periodStart/periodEnd; keep fallback to from/to)
  const from = reportDoc?.from ?? reportDoc?.periodStart ?? "";
  const to = reportDoc?.to ?? reportDoc?.periodEnd ?? "";

  // Snapshot HTML (we only render the table part on desktop)
  const rawHtml = reportDoc?.payload?.html || reportDoc?.html || "";
  const html = extractTableFromSnapshot(rawHtml);

  // For mobile summary totals computed live for the saved period
  ...
  return (
    <>
      {/* Desktop: show ONLY the saved snapshot table */}
      <div className="hidden sm:block">
        {html ? (
          <div
          ...
          <div className="text-ink/60">No snapshot found.</div>
        )}
      </div>

      {/* Mobile: compact totals card */}
      <div className="sm:hidden space-y-2">
        <div className="card p-3">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
            <div>
              Debit Total: <span className="font-mono">{fmt(totals.debit)}</span>
            </div>
            <div>
              Credit Total:{" "}
          ...
          <div className="mt-2 text-sm">
            {isBalanced ? (
              ...
              <span className="text-rose-700 font-semibold">
                ⚠️ Out of balance — Diff:{" "}
                <span className="font-mono">{fmt(totals.diff)}</span>
              </span>
            )}
          </div>
        </div>
      ...
      {/* Actions (we keep export via print from the browser or CSV/PDF if needed later) */}
      <div className="mt-2 flex gap-2">
        <button className="btn btn-outline" onClick={() => window.print()}>
          Print
        </button>
...
export default function ReportView() {
  ...
  const type = (reportDoc?.type || "").toLowerCase();
    ...
    >
      ...
      {loading ? (
        ...
        <div className="card p-4">
          ...
          {type === "cash_flow" && <CashFlowView reportDoc={reportDoc} />}
          {type === "trial_balance" && (
            <TrialBalanceView reportDoc={reportDoc} />
          )}

          {/* Fallback: if unknown type, try to render payload.html */}
          {!["income_statement", "balance_sheet", "cash_flow", "trial_balance"].includes(
            type
          ) && reportDoc?.payload?.html && (
            <div
              className="overflow-x-auto"
              dangerouslySetInnerHTML={{ __html: reportDoc.payload.html }}
            />
          )}
        </div>
      )}
